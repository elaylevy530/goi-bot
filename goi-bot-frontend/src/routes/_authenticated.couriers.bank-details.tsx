...
function BankDetailsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const couriers = await nestListCouriers();
      return couriers
        .map((c) => ({
          id: c.id,
          full_name: c.full_name ?? null,
          phone: c.whatsapp_phone ?? null,
          bank_name: (c as Record<string, unknown>).bank_name as string | null ?? null,
          bank_branch: (c as Record<string, unknown>).bank_branch as string | null ?? null,
          bank_account: (c as Record<string, unknown>).bank_account as string | null ?? null,
          bank_account_owner: (c as Record<string, unknown>).bank_account_owner as string | null ?? null,
          bank_details_verified: (c as Record<string, unknown>).bank_details_verified as boolean | null ?? null,
  ...
  const setVerified = useMutation({
    mutationFn: async ({ id, verified }: { id: string; verified: boolean }) => {
      await nestUpdateCourier(id, { bank_details_verified: verified });
    },
    onSuccess: () => {

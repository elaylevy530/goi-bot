...
function RatingsPage() {
  ...
  const { data: reviews = [] } = useQuery({
    ...
    enabled: !!me?.id,
    queryFn: async () => {
      const data = await nestListMyCourierOutcomes();
      return (data ?? []).filter((o) => o.customer_rating != null).slice(0, 30);
    },
  });

  const completionRate = stats ? (Number(stats.jobs_completed) / Math.max(1, Number(stats.jobs_completed) + Number(stats.jobs_cancelled)) * 100).toFixed(0) : "—";
    ...
    <CourierShell title="דירוגים וביצועים" subtitle="הביצועים שלך לאורך זמן">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "זמן תגובה ממוצע", value: fmtSec(Number(stats?.avg_response_seconds ?? 0)) },
        ].map((k) => (

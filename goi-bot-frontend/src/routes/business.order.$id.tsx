...
function OrderDetailPage() {
  ...
  const courierId = (jobRaw as any)?.selected_courier_id as string | undefined;

  const { data: courier } = useQuery({
    queryKey: ["order-courier", courierId],
    enabled: !!courierId,
    refetchInterval: 30_000,
    queryFn: () => nestGetCourier(courierId!),
  });

  const job = jobRaw ? { ...(jobRaw as any), couriers: courier ?? null } : null;

  const { data: courierStats } = useQuery({
    queryKey: ["courier-stats", courierId],
    enabled: !!courierId,
    queryFn: () => nestGetCourierStats(courierId!),
  });

  const { data: favoriteRaw } = useQuery({
    queryKey: ["favorite", me?.id, courierId],
    enabled: !!me?.id && !!courierId,
    queryFn: () => nestGetFavoriteCourier(courierId!),
  });

  const favorite = favoriteRaw
    ? {
        id: String((favoriteRaw as any).id),
        status: ((favoriteRaw as any).status === "blocked"
          ? "blocked"
          : "preferred") as "preferred" | "blocked",
      }
    : null;
  ...
  const toggleFavorite = useMutation({
    mutationFn: async (next: "preferred" | "blocked" | "none") => {
      ...
      if (next === "none") {
        ...
        await nestSetFavoriteCourier(courierId, next);
      }
    },
    onSuccess: (_d, next) => {
  ...
  const { data: logs } = useQuery({
    ...
    enabled: !!job,
    refetchInterval: 5_000,
    queryFn: () => nestListJobStatusLogs(id),
  });

  const cancel = useMutation({
    mutationFn: async () => {
      if (!me) throw new Error("חסר פרופיל");
      await nestUpdateJob(id, { status: "בוטלה" });
    },
  ...
  const duplicate = useMutation({
    mutationFn: async () => {
      ...
      const { id: _i, created_at, updated_at, job_number, status, selected_courier_id, selected_quote_id, couriers, ...rest } = job as any;
  ...
  const saveRecipient = useMutation({
    mutationFn: async () => {
      ...
      await nestUpsertSavedContact({
        contact_name: j.recipient_name,
        phone: j.recipient_phone || null,
        city: j.dropoff_area || null,
        full_address: j.dropoff_address || null,
        notes: j.dropoff_notes || null,
      });
    },
  ...
  const submitRating = useMutation({
    mutationFn: async () => {
      ...
      await nestPutJobOutcome(id, {
        courier_id: (job as any).selected_courier_id,
        customer_rating: rating,
        customer_comment: feedback || null,
      });
      if (tip > 0) {
        await nestUpdateJob(id, { tip_amount: tip });
      }
    },

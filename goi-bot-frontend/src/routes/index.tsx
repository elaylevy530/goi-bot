...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "אפליקציית Goi — משלוחים והובלות ישירות מהמסך שלכם" },
      {
        name: "description",
        content:
          "הזמינו משלוח מהיר או הובלת רהיטים ישירות מהאפליקציה של Goi. מפה חיה, צ׳אט עם השליח, תשלום מאובטח — הכל במסך אחד, בלי חנות אפליקציות.",
      },
      { property: "og:title", content: "אפליקציית Goi — משלוחים והובלות מהמסך שלכם" },
      {
        property: "og:description",
        content:
          "משלוחים והובלות בכמה נגיעות. מפה חיה, צ׳אט וקבלה — הכל באפליקציה של Goi.",
      },
      ...
      >
        <div className="max-w-[1240px] mx-auto flex items-center justify-between gap-2 px-4 sm:px-5 lg:px-10 h-16">
          ...
          <nav className="hidden md:flex items-center gap-6 lg:gap-7 text-[14px] text-white/80">
            ...
            <a href="#delivery" className="hover:text-white transition">משלוחים</a>
            <a href="#moving" className="hover:text-white transition">הובלות</a>
    ...
    <section className="relative overflow-hidden bg-[#0A0B0D] text-white">
      ...
      <div className="relative max-w-[1240px] mx-auto px-4 sm:px-5 lg:px-10 pt-24 sm:pt-28 lg:pt-32 pb-6 lg:pb-10 grid lg:grid-cols-[1.05fr_0.95fr] gap-6 sm:gap-10 lg:gap-14 items-center">
        <div className="text-center lg:text-right">
          ...
          >
            צריך שליח, איסוף
            <span className="block mt-1">או הובלה קטנה?</span>
          ...
          <p className="mt-4 lg:mt-7 text-[15px] sm:text-[18px] lg:text-[19.5px] leading-[1.6] sm:leading-[1.7] text-white/60 max-w-[560px] mx-auto lg:mx-0">
            <span className="font-semibold text-white/90">אלפי שליחים ומובילים זמינים, במקום אחד.</span>
            <span className="block mt-1.5 sm:mt-2">קובעים מחיר או בוחרים הצעה שמתאימה לכם.</span>
            <span className="block mt-2 sm:mt-3 font-semibold text-white/90">בלי טלפונים. בלי קבוצות פייסבוק. פשוט מזמינים.</span>
          </p>


          <div className="mt-5 lg:mt-10 flex flex-col sm:flex-row items-center gap-3 sm:gap-4 justify-center lg:justify-start">
            <a
              href="#install"
              className="group relative inline-flex items-center gap-3 h-14 sm:h-16 pl-3 pr-5 sm:pr-6 rounded-full font-bold text-[15px] sm:text-[16px] overflow-hidden transition active:scale-[0.98]"
              style={{
                background: `linear-gradient(135deg, ${BRAND} 0%, #2b8f22 100%)`,
                color: "white",
                boxShadow: `0 25px 60px -15px ${BRAND}, 0 0 0 1px rgba(255,255,255,0.12) inset, 0 1px 0 rgba(255,255,255,0.25) inset`,
              }}
            >
              <span className="absolute inset-0 pointer-events-none"
                style={{ background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.28), transparent)", transform: "translateX(-100%)", animation: "shineSweep 2.4s ease-in-out infinite" }} />
              <span className="relative">התקינו את האפליקציה</span>
              <span className="relative size-10 sm:size-11 rounded-full bg-white/15 backdrop-blur grid place-items-center border border-white/20 group-hover:bg-white/25 transition">
                <Download className="w-5 h-5" strokeWidth={2.4} />
              </span>
            </a>
            <span className="sm:hidden text-[12.5px] text-white/45">30 שניות ואתם בפנים ✨</span>
          </div>

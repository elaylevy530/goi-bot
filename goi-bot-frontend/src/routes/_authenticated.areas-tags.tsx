...
function AreasTagsPage() {
  ...
  const addArea = useMutation({
    mutationFn: async () => {
      throw new Error("TODO Nest: area create endpoint is not available");
    },
    onSuccess: () => { setNewArea(""); qc.invalidateQueries({ queryKey: ["areas-mgmt"] }); toast.success("אזור נוסף"); },
    onError: (e: Error) => toast.error(e.message),
  });
  const removeArea = useMutation({
    mutationFn: async (_id: string) => { throw new Error("TODO Nest: area delete endpoint is not available"); },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["areas-mgmt"] }),
  });
  const addTag = useMutation({
    mutationFn: async () => { throw new Error("TODO Nest: tags endpoint is not available"); },
    onSuccess: () => { setNewTag(""); qc.invalidateQueries({ queryKey: ["tags"] }); toast.success("תגית נוספה"); },
    onError: (e: Error) => toast.error(e.message),
  });
  const removeTag = useMutation({
    mutationFn: async (_id: string) => { throw new Error("TODO Nest: tags endpoint is not available"); },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["tags"] }),
  });
  const toggleRule = useMutation({
    mutationFn: async (_input: { id: string; enabled: boolean }) => { throw new Error("TODO Nest: classification rules endpoint is not available"); },

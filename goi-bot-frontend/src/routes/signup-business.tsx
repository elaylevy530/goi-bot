...
import { nestHomePath, nestRegisterBusiness } from "@/lib/nest-auth";
...
function SignupBusinessPage() {
  ...
  const mutate = useMutation({
    ...
    onSuccess: (session) => {
      toast.success("החשבון נפתח!");
      navigate({ to: nestHomePath(session), replace: true });
    },
    onError: (e: Error) => {
      toast.error(e instanceof ApiClientError ? e.message : e.message);
    },
  });

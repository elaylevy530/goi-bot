...
function WithdrawalsPage() {
  ...
  const filtered = filter === "all" ? typedRows : typedRows.filter((r) => r.status === filter);

  const totals = {
    pending: typedRows.filter((r) => r.status === "ממתינה").reduce((s, r) => s + Number(r.amount), 0),
    approved: typedRows.filter((r) => r.status === "אושרה").reduce((s, r) => s + Number(r.amount), 0),
    paid: typedRows.filter((r) => r.status === "שולמה").reduce((s, r) => s + Number(r.amount), 0),
  };

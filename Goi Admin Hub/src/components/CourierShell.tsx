...
function useCourierNavCounts(courier?: any | null) {
  ...
  return useQuery<CourierNavCounts>({
    ...
    queryFn: async () => {
      ...
      const jobCols = "id, status, selected_courier_id, pricing_type, service_category, job_type, package_type, item_category, job_date, pickup_area, pickup_address, pickup_lat, pickup_lng, vehicle_required";
      const [pendingRes, openRes, activeRes, declinedRes] = await Promise.all([
        supabase
          .from("offer_events")
          .select(`id, response, expires_at, jobs(${jobCols})`)
        ...
        supabase
          ...
          .eq("selected_courier_id", courierId!)
      ...
      const declined = new Set(((declinedRes.data ?? []) as any[]).map((r) => r.job_id));
      const unique = new Set<string>();
      for (const o of (pendingRes.data ?? []) as any[]) {
        if (isLivePendingOffer(o, courier)) {
          const jobId = Array.isArray(o.jobs) ? o.jobs[0]?.id : o.jobs?.id;
          if (jobId && !declined.has(jobId)) unique.add(jobId);
        }
      }
      for (const j of (openRes.data ?? []) as any[]) {
...
export function CourierShell({ children, title, subtitle, headerExtra, fullBleed = false }: {
  ...
  // Realtime sync — panel updates immediately after dashboard changes and on job/offer activity.
  useEffect(() => {
    if (!me?.id) return;
    const channel = supabase
      .channel(`courier-shell-${me.id}`)
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "couriers", filter: `id=eq.${me.id}` },
        () => qc.invalidateQueries({ queryKey: ["my-courier-me"] }),
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "offer_events", filter: `courier_id=eq.${me.id}` },
  ...
  useEffect(() => {
    ...
    const showJobAlert = async (jobId: string) => {
      ...
      const { data: job } = await supabase
        .from("jobs")
        .select("id, job_number, job_type, package_type, item_category, pickup_area, pickup_address, dropoff_area, dropoff_address, payment, suggested_courier_payment, estimated_distance_km")
        .eq("id", jobId)
        .maybeSingle();
      if (!job) return;
      const pickup = String((job as any).pickup_area || (job as any).pickup_address || "איסוף").split(",")[0].trim();
      const dropoff = String((job as any).dropoff_area || (job as any).dropoff_address || "מסירה").split(",")[0].trim();
      const type = (job as any).item_category || (job as any).package_type || (job as any).job_type || "משלוח";
      const price = (job as any).suggested_courier_payment ?? (job as any).payment;
    ...
    const channel = supabase
      .channel(`courier-heads-up-${me.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "offer_events", filter: `courier_id=eq.${me.id}` }, (payload) => {
        const jobId = (payload.new as { job_id?: string }).job_id;
        if (jobId) void showJobAlert(jobId);
      })
      .subscribe((status) => {

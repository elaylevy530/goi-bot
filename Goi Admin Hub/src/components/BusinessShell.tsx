...
export function useMyBusiness() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("customers").select("*").eq("user_id", u.user.id).maybeSingle();
      return data;
    },
    staleTime: 10_000,
  });
...
function useUnreadNotifications(businessId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { count } = await supabase.from("business_notifications" as never)
        .select("id", { count: "exact", head: true })
        .eq("business_id", businessId!).is("read_at", null);
      return count ?? 0;
    },
  });
}
...
function useRecentNotifications(businessId?: string) {
  return useQuery({
    ...
    refetchInterval: 90_000,
    queryFn: async () => {
      const { data } = await supabase.from("business_notifications" as never)
...
function NotificationsBell({ businessId }: { businessId?: string }) {
  ...
  const markAll = useMutation({
    mutationFn: async () => {
      ...
      await supabase.from("business_notifications" as never)
    ...
    <Popover>
      ...
      <PopoverContent dir="rtl" align="end" className="w-80 p-0 rounded-2xl overflow-hidden">
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          ) : (items as Array<{ id: string; link?: string; title: string; body?: string; read_at?: string | null; created_at: string }>).map((n) => (
            <Link
              key={n.id}
...
export function BusinessShell({ children, title, subtitle, headerExtra }: {
  ...
  useEffect(() => {
    if (!me?.id) return;
    const ch = supabase.channel(`biz-notif-${me.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "business_notifications", filter: `business_id=eq.${me.id}` }, () => {

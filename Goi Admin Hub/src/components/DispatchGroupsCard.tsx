...
export function DispatchGroupsCard() {
  ...
  const getPhoneFn = useServerFn(getAdminNotifyPhone);
  const setPhoneFn = useServerFn(setAdminNotifyPhone);
  ...
  const phoneQ = useQuery({ queryKey: ["admin-notify-phone"], queryFn: () => getPhoneFn() });
        ...
        <div className="space-y-2 border-t pt-4">
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="max-w-xs"
            />
            <Button variant="outline" onClick={() => savePhoneM.mutate()} disabled={savePhoneM.isPending}>

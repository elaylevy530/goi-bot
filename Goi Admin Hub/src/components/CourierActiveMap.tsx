...
export function CourierActiveMap() {
  ...
  const openBusinessChat = async (job: ActiveJob | null | undefined) => {
    ...
    try {
      ...
      if (error || !convId) throw error ?? new Error("no conv");
      navigate({ to: "/courier/messages", search: { c: convId as string } as never });
    } catch (e: any) {
      toast.error(e?.message ?? "לא ניתן לפתוח צ׳אט עם העסק");
    }
  };
  ...
  useEffect(() => {
    if (!me?.id) return;
    const ch = supabase
      .channel(`courier-active-map-${me.id}`)
  ...
  const { data: jobsRaw = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("jobs")
        .select("id, job_number, job_type, status, customer_id, pickup_area, pickup_address, pickup_lat, pickup_lng, pickup_contact_name, pickup_contact_phone, pickup_instructions, pickup_notes, pickup_ready, pickup_ready_at, dropoff_area, dropoff_address, dropoff_building, dropoff_entrance, dropoff_floor, dropoff_apartment, dropoff_notes, dropoff_lat, dropoff_lng, recipient_name, recipient_phone, job_date, job_time, payment, customer_name, description, number_of_packages, item_category, package_type, job_outcomes(picked_up_at, delivered_at)")
  ...
  const { data: lastSteps = {} } = useQuery({
    queryKey: ["active-job-steps-map", jobsRaw.map((j) => j.id).join(",")],

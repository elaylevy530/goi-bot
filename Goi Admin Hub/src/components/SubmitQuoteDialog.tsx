...
export function SubmitQuoteDialog({ open, onOpenChange, jobId, jobNumber, existing }: Props) {
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase.rpc("submit_job_quote", {
        _job_id: jobId,
        _price: p,
        _estimated_arrival_minutes: arrival ? Number(arrival) : undefined,
        _estimated_delivery_minutes: delivery ? Number(delivery) : undefined,
        _note: note || undefined,
        _includes_invoice: invoice,
        _is_final_price: isFinal,
      });

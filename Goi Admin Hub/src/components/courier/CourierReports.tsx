...
export function CourierReports() {
  ...
  const { data: outcomes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("job_outcomes")
        .select("id, delivered_at, was_cancelled, was_late, customer_rating, tip_amount, jobs(job_number, job_type, pickup_area, dropoff_area, payment, customer_name)")
        .eq("courier_id", me!.id)
  ...
  const done = outcomes.filter((o: any) => !o.was_cancelled);
  const totalEarned = done.reduce((s: number, o: any) => s + Number(o.jobs?.payment ?? 0), 0);
  ...
  const avgRating = rated.length ? (rated.reduce((s: number, o: any) => s + Number(o.customer_rating), 0) / rated.length) : null;
  const lateCount = outcomes.filter((o: any) => o.was_late).length;

...
function PauseCourierButton({ id, isPaused, status }: { id: string; isPaused: boolean; status?: string | null }) {
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("couriers").update(patch as never).eq("id", id);
...
function EditCourierDialog({ courier }: { courier: any }) {
  ...
  const { data: areas = [] } = useQuery({
    queryKey: ["areas-list"],
    queryFn: async () => (await supabase.from("areas").select("name").eq("is_active", true).order("name")).data ?? [],
  });
  ...
  const mut = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("couriers").update({
        ...
        vehicle_type: form.vehicle_type as never,
        invoice_status: form.invoice_status as never,
        experience: form.experience || null,
        courier_status: form.courier_status,
        working_areas: form.working_areas,
        job_types: form.job_types as never,
...
function TagsCard({ courierId }: { courierId: string }) {
  ...
  const { data: courierTags = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("courier_tags")
        .select("tag_id, assigned_automatically, tags(id, name, color, is_automatic)")
  ...
  const addTag = useMutation({
    mutationFn: async (tag_id: string) => {
      const { error } = await supabase.from("courier_tags").insert({ courier_id: courierId, tag_id, assigned_automatically: false });
      if (error) throw error;
    },
  ...
  const removeTag = useMutation({
    mutationFn: async (tag_id: string) => {
      const { error } = await supabase.from("courier_tags").delete().eq("courier_id", courierId).eq("tag_id", tag_id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["courier-tags", courierId] }),
  });
...
function CourierProfile() {
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: async () => (await supabase.from("whatsapp_messages").select("*").eq("courier_id", id).order("created_at", { ascending: false }).limit(50)).data ?? [],
  });
  const { data: jobsHistory = [] } = useQuery({
    queryKey: ["courier-jobs", id],
    queryFn: async () => (await supabase.from("jobs").select("job_number, job_type, status, payment, created_at").eq("selected_courier_id", id).order("created_at", { ascending: false }).limit(50)).data ?? [],
  });
  const { data: logs = [] } = useQuery({
    queryKey: ["courier-logs", id],
    queryFn: async () => (await supabase.from("status_logs").select("*").eq("entity_type", "courier").eq("entity_id", id).order("created_at", { ascending: false }).limit(50)).data ?? [],
  });

  useEffect(() => {
    if (!id) return;
    const ch = supabase
      .channel(`admin-courier-${id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "couriers", filter: `id=eq.${id}` }, () => {
        qc.invalidateQueries({ queryKey: ["courier", id] });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "jobs", filter: `selected_courier_id=eq.${id}` }, () => {
        qc.invalidateQueries({ queryKey: ["courier-jobs", id] });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "whatsapp_messages", filter: `courier_id=eq.${id}` }, () => {
        qc.invalidateQueries({ queryKey: ["courier-messages", id] });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "status_logs", filter: `entity_id=eq.${id}` }, () => {
        qc.invalidateQueries({ queryKey: ["courier-logs", id] });
      })
      .subscribe();
  ...
  const saveNotes = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("couriers").update({ notes }).eq("id", id);
      if (error) throw error;
    },

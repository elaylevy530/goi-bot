...
function CustomerDashboard() {
  ...
  const { identity, orders: guestRefs } = useGuestSession();

  useEffect(() => {
    if (isGuest) {
      const channels = guestRefs.slice(0, 10).map((r) =>
        supabase
          .channel(`guest-jobs-${r.job_id}`)
    ...
    (async () => {
      ...
      const u = data.user;
      if (!u) return;
      const meta = (u.user_metadata ?? {}) as { phone?: string; full_name?: string };
      const phone = meta.phone || u.email?.split("@")[0] || "";
      if (!phone) return;
      channel = supabase
        .channel(`my-jobs-${phone}`)
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "jobs", filter: `guest_phone=eq.${phone}` },
          () => qc.invalidateQueries({ queryKey: ["my-orders"] }),
        )
        .subscribe();
    })();

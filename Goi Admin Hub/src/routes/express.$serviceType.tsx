...
function ExpressOrderPage() {
  ...
  // Prefill from signed-in customer account (if any)
  useEffect(() => {
    (async () => {
      const { data } = await supabase.auth.getUser();
      const u = data.user;
      if (!u?.email?.endsWith("@customers.goi.local")) return;
      const meta = (u.user_metadata ?? {}) as { full_name?: string; phone?: string };
      const phoneFromEmail = u.email.split("@")[0];
      setForm((f) => ({
        ...f,
        guest_name: f.guest_name || meta.full_name || "",
        guest_phone: f.guest_phone || meta.phone || phoneFromEmail || "",
      }));
    })();
  }, []);

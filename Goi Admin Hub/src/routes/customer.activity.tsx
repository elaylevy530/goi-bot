...
import { useGuestSession, guestTokenFor } from "@/lib/guest-session";
...
function ActivityPage() {
  ...
  const { orders: guestRefs } = useGuestSession();
  const [q, setQ] = useState("");

  useEffect(() => {
    if (isGuest) {
      // Guests: subscribe per known job id (no phone-scoped filter available)
      const channels = guestRefs.slice(0, 10).map((r) =>
        supabase
          .channel(`guest-activity-${r.job_id}`)
          .on("postgres_changes", { event: "*", schema: "public", table: "jobs", filter: `id=eq.${r.job_id}` },
    ...
    (async () => {
      ...
      const meta = (data.user.user_metadata ?? {}) as { phone?: string };
      const phone = meta.phone || data.user.email?.split("@")[0] || "";
      if (!phone) return;
      channel = supabase
        .channel(`activity-${phone}`)
        .on("postgres_changes", { event: "*", schema: "public", table: "jobs", filter: `guest_phone=eq.${phone}` },
          () => qc.invalidateQueries({ queryKey: ["my-orders"] }))
        .subscribe();
    })();
    return () => { cancelled = true; if (channel) supabase.removeChannel(channel); };
  }, [qc, isGuest, guestRefs]);

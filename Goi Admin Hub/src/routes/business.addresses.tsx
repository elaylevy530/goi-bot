...
function AddressesPage() {
  ...
  const { data: list } = useQuery({
    ...
    enabled: !!me?.id,
    queryFn: async () => {
      const { data } = await supabase.from("business_branches").select("*").eq("business_id", me!.id).order("is_default", { ascending: false });
      return data ?? [];
    },
  });
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (f.is_default) {
        await supabase.from("business_branches").update({ is_default: false }).eq("business_id", me.id);
      }
      if (editing) {
        const { error } = await supabase.from("business_branches").update({ ...f }).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("business_branches").insert({ ...f, business_id: me.id });
        if (error) throw error;
      }
    },
  ...
  const del = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("business_branches").delete().eq("id", id); if (error) throw error; },
  ...
  const setDefault = useMutation({
    mutationFn: async (id: string) => {
      if (!me) return;
      await supabase.from("business_branches").update({ is_default: false }).eq("business_id", me.id);
      await supabase.from("business_branches").update({ is_default: true }).eq("id", id);
    },

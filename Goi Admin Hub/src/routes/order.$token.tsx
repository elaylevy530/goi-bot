...
function PublicOrderPage() {
  ...
  // Look up business display name from token
  const { data: meta } = useQuery({
    queryKey: ["order-token-meta", token],
    queryFn: async () => {
      const { data } = await supabase
        .from("business_integrations")
        .select("business_id, enabled, customers(name, business_name)")
  ...
  const businessName =
    (meta as any)?.customers?.business_name || (meta as any)?.customers?.name || "העסק";

...
function AreasTagsPage() {
  ...
  const { data: areas = [] } = useQuery({
    queryKey: ["areas-mgmt"],
    queryFn: async () => (await supabase.from("areas").select("*").order("name")).data ?? [],
  });
  ...
  const { data: rules = [] } = useQuery({
    queryKey: ["rules"],
    queryFn: async () => (await supabase.from("classification_rules").select("*, tags(name)").order("created_at")).data ?? [],
  });

  const addArea = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("areas").insert({ name: newArea });
  ...
  const removeArea = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("areas").delete().eq("id", id); if (error) throw error; },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["areas-mgmt"] }),
  });
  const addTag = useMutation({
    mutationFn: async () => { const { error } = await supabase.from("tags").insert({ name: newTag }); if (error) throw error; },
  ...
  const toggleRule = useMutation({
    mutationFn: async ({ id, enabled }: { id: string; enabled: boolean }) => {
      const { error } = await supabase.from("classification_rules").update({ enabled }).eq("id", id);
      if (error) throw error;
    },

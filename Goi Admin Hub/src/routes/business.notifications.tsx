...
function NotificationsPage() {
  ...
  const { data: items } = useQuery({
    ...
    enabled: !!me?.id,
    queryFn: async () => {
      const { data } = await supabase.from("business_notifications" as any)
  ...
  const markRead = useMutation({
    mutationFn: async (id?: string) => {
      const q = supabase.from("business_notifications" as any).update({ read_at: new Date().toISOString() } as any).eq("business_id", me!.id).is("read_at", null);
      if (id) q.eq("id", id);
      const { error } = await q;

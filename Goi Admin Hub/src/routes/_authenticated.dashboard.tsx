...
async function loadStats() {
  ...
  const [
    couriersAll, couriersToday, couriersActive, couriersPending,
    customersAll, jobsOpen, jobsToday,
    missingDetails, newCustomers, unassignedJobs,
    pendingWithdrawals,
  ] = await Promise.all([
    supabase.from("couriers").select("id", { head: true, count: "estimated" }),
    supabase.from("couriers").select("id", { head: true, count: "estimated" }).gte("created_at", today),
    supabase.from("couriers").select("id", { head: true, count: "estimated" }).eq("courier_status", "פעיל"),
    supabase.from("couriers").select("id", { head: true, count: "estimated" }).eq("courier_status", "ממתין לאישור"),
    supabase.from("customers").select("id", { head: true, count: "estimated" }),
    supabase.from("jobs").select("id", { head: true, count: "estimated" }).not("status", "in", "(הושלמה,בוטלה)"),
    supabase.from("jobs").select("id", { head: true, count: "estimated" }).gte("created_at", today).neq("status", "טיוטה"),
    supabase.from("couriers").select("id", { head: true, count: "estimated" }).eq("courier_status", "חסר פרטים"),
    supabase.from("customers").select("id", { head: true, count: "estimated" }).eq("status", "חדש"),
    supabase.from("jobs").select("id", { head: true, count: "estimated" }).is("selected_courier_id", null).neq("status", "טיוטה"),
    supabase.from("withdrawal_requests").select("id", { head: true, count: "estimated" }).eq("status", "ממתינה"),
  ]);


  return {
    stats: {
      total_couriers: couriersAll.count ?? 0,
      registered_today: couriersToday.count ?? 0,
      active_couriers: couriersActive.count ?? 0,
      pending_approval: couriersPending.count ?? 0,
      total_customers: customersAll.count ?? 0,
      open_jobs: jobsOpen.count ?? 0,
      jobs_sent_today: jobsToday.count ?? 0,
      courier_replies_today: 0,
    },
    attention: [
      { label: "שליחים שחסר להם מידע", count: missingDetails.count ?? 0, to: "/couriers" },
      { label: "שליחים חדשים לא מאושרים", count: couriersPending.count ?? 0, to: "/couriers" },
      { label: "מזמינים חדשים", count: newCustomers.count ?? 0, to: "/customers" },
      { label: "עבודות בלי שליח", count: unassignedJobs.count ?? 0, to: "/jobs" },
      { label: "בקשות משיכה ממתינות", count: pendingWithdrawals.count ?? 0, to: "/withdrawals" },
  ...
  const { data: pending = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("couriers")
        .select("id, full_name, whatsapp_phone, base_city, vehicle_label, vehicle_type, working_areas, job_types, max_distance, delivery_bag, invoice_status, created_at")
        .eq("courier_status", "ממתין לאישור")

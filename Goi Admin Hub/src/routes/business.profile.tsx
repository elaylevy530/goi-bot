...
function ProfilePage() {
  ...
  const { data: logoUrl } = useQuery({
    queryKey: ["business-logo-preview", logoPath],
    enabled: !!logoPath,
    staleTime: 1000 * 60 * 30,
    queryFn: async () => {
      if (!logoPath) return null;
      const { data } = await supabase.storage.from("business-logos").createSignedUrl(logoPath, 60 * 60);
  ...
  async function onPickLogo(file: File) {
    if (!me) return;
    if (!file.type.startsWith("image/")) { toast.error("קובץ תמונה בלבד"); return; }
    if (file.size > 5 * 1024 * 1024) { toast.error("עד 5MB"); return; }
    try {
      setUploading(true);
      ...
      const { error: dbErr } = await supabase.from("customers").update({ logo_url: path } as any).eq("id", me.id);
  ...
  async function onRemoveLogo() {
    ...
    try {
      setUploading(true);
      await supabase.storage.from("business-logos").remove([logoPath]);
      const { error } = await supabase.from("customers").update({ logo_url: null } as any).eq("id", me.id);

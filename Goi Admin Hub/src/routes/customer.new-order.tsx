...
// Specific arrival hours for movers (customer picks an exact time)
const MOVE_WINDOWS: { start: string; label: string }[] = [
  "06:00", "07:00", "08:00", "09:00", "10:00", "11:00",
  "12:00", "13:00", "14:00", "15:00", "16:00", "17:00",
  "18:00", "19:00", "20:00", "21:00",
].map((t) => ({ start: t, label: t }));
...
function readStickyPartnerSlug(): string | null {
  try {
...
function NewOrderPage() {
  ...
  const { service: initialService, p: partnerSlugParam } = Route.useSearch();
  // The partner panel (/p/:slug) redirects here with ?p=slug. A refresh, a
  // back/forward step or an internal navigation can drop that param, which
  // used to detach the order from the partner (and from its WhatsApp group).
  // Remember the slug for this browser so partner orders stay attributed.
  const [partnerSlug, setPartnerSlug] = useState<string | null>(partnerSlugParam ?? null);
  useEffect(() => {
    if (partnerSlugParam) {
      try { window.localStorage.setItem(PARTNER_SLUG_KEY, partnerSlugParam); } catch {}
      setPartnerSlug(partnerSlugParam);
      return;
    }
    const sticky = readStickyPartnerSlug();
    if (sticky) setPartnerSlug(sticky);
  }, [partnerSlugParam]);
  ...
  const [pricingModel, setPricingModel] = useState<"fixed_price" | "quote_request">("fixed_price");
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!u) {
        ...
        if (g) {
          setProfile({ full_name: g.full_name, phone: g.phone });
      ...
      setProfile({
        full_name: meta.full_name ?? "לקוח",
        phone: meta.phone ?? phoneFromEmail,
      });
    })();
  }, []);
  ...
  const { data: featureFlags } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("key, value")
        .in("key", ["munch_enabled", "send_enabled", "bring_enabled"]);
      const read = (k: string) => {
      ...
      return {
        munch: read("munch_enabled"),
        send: read("send_enabled"),
        bring: read("bring_enabled"),
      };
    },
  });
  ...
  const rule = useMemo(() => (rules ?? []).find((r: any) => r.service_category === service), [rules, service]);
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      if (pricingModel === "fixed_price" && !offeredPrice) throw new Error("הזן מחיר שאתה מציע");
      ...
      if (isMoveCat) {
        if (moveWhen === "today" && !moveTodayFrom)
          throw new Error("בחר שעה להובלה היום");
        if (moveWhen === "scheduled" && (!moveDate || !moveFrom))
          throw new Error("בחר תאריך ושעה להובלה");
      }
  ...
  async function handlePhotoUpload(file: File) {
    ...
    try {
      ...
      const { error } = await supabase.storage.from("guest-order-photos").upload(path, file, {
        cacheControl: "3600", upsert: false, contentType: file.type || "image/jpeg",
      });
      if (error) throw error;
      const { data: signed } = await supabase.storage.from("guest-order-photos").createSignedUrl(path, 3600);
  ...
  async function removePhoto(path: string) {
    ...
    void supabase.storage.from("guest-order-photos").remove([path]);
  }
  ...
  const guestPhoneOk = guestPhone.replace(/[^0-9+]/g, "").length >= 9;
  const guestContactOk = guestNameOk && guestPhoneOk;
  const canContinue = !!pickup && !!dropoff && (isGuestMode ? (isMunch ? true : guestContactOk) : !!profile.phone);
  // Guests submit straight from the form — details are collected inline
  const startOrder = () => {
    if (isGuestMode) {
      const name = guestName.trim();
      const phone = guestPhone.replace(/[^0-9+]/g, "");
      if (!guestContactOk) {
        toast.error("הזן שם מלא ומספר טלפון");
        return;
      }
    ...
    <div className="fixed inset-0 bottom-16 md:bottom-0 flex flex-col bg-[#f5f6f8]">
      ...
      {guestDialog && (
        <div className="fixed inset-0 z-50 bg-black/50 grid place-items-end sm:place-items-center p-0 sm:p-4" dir="rtl">
          <div className="w-full sm:max-w-sm bg-white rounded-t-3xl sm:rounded-3xl p-5 space-y-3 max-h-[85dvh] overflow-y-auto pb-[calc(1.25rem+env(safe-area-inset-bottom))]">
            <div>
      ...
      >
        ...
        <div className="flex-shrink-0 px-3 pt-2 pb-1">
          {!expanded && (
            <div className="mb-2 px-1">
              {partner && (
                <div className="mb-1.5 inline-flex items-center gap-1.5 rounded-full bg-[#eef7f0] ring-1 ring-emerald-600/20 px-2.5 py-1 text-[11px] font-bold text-emerald-800">
                  🤝 בשיתוף {partner.name}
                </div>
              )}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-3 space-y-3">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-1.5 mb-2">
                {([
                  ...
                  { key: "today", label: "היום", sub: "שעה מדויקת" },
                  { key: "scheduled", label: "מתוזמן", sub: "תאריך + שעה" },
                ] as const).map((opt) => {
              ...
              {moveWhen === "today" && (
                <div>
                  <div className="text-[11px] text-[#101418]/60 mb-1.5">בחר שעה</div>
                  <div className="grid grid-cols-4 gap-1.5">
                    {MOVE_WINDOWS.map((w) => {
              ...
              {moveWhen === "scheduled" && (
                <div className="space-y-2">
                  ...
                  <div>
                    <div className="text-[11px] text-[#101418]/60 mb-1.5">שעה</div>
                    <div className="grid grid-cols-4 gap-1.5">
                      {MOVE_WINDOWS.map((w) => {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-1.5 mb-2">
                ...
                {rule.allow_customer_quote && (
                  <button
                    type="button"
                    onClick={() => setPricingModel("quote_request")}
                    className={`py-2 rounded-xl text-xs font-bold transition ${
          ...
          {/* Guest contact — collected inline, no popup */}
          {isGuestMode && !isMunch && (
            <div className="rounded-2xl bg-white ring-1 ring-black/5 p-3.5 space-y-2.5">
              <div>
                <div className="text-sm font-extrabold">פרטי קשר</div>
                <p className="text-[11px] text-[#101418]/60 mt-0.5">
                  רק שם וטלפון כדי שהמוביל יוכל ליצור איתך קשר. לא צריך להירשם.
                </p>
              </div>
              <input
          ...
          {!isMunch && (() => {
            const missingReason = !canContinue
              ? (!pickup ? "בחר נקודת איסוף כדי להמשיך" : !dropoff ? "בחר יעד כדי להמשיך" : isGuestMode ? (!guestNameOk ? "הזן שם מלא כדי להמשיך" : "הזן מספר טלפון כדי להמשיך") : "הוסף מספר טלפון כדי להמשיך")
            ...
            return (
              <div className="pt-4 pb-2">
                ...
                <label className="mt-2 flex items-start gap-2 rounded-xl bg-[#f5f6f8] px-3 py-2 cursor-pointer select-none">
                  <input
                    type="checkbox"
        ...
        {expanded && (() => {
          ...
          const missingReason = !canContinue
            ? (!pickup ? "בחר נקודת איסוף כדי להמשיך" : !dropoff ? "בחר יעד כדי להמשיך" : isGuestMode && !isMunch ? (!guestNameOk ? "הזן שם מלא כדי להמשיך" : "הזן מספר טלפון כדי להמשיך") : "הוסף מספר טלפון כדי להמשיך")
            : !termsAccepted ? "סמן את אישור תנאי השירות"
            : null;
          ...
          const ctaNode = (
            <div className={wrapperClass}>
              <button
                type="button"
                onClick={() => (isMunch && isGuestMode && !guestContactOk ? setGuestDialog(true) : startOrder())}
...
function extraNotes(desc: string): string {
  ...
  // Keep the user's spacing intact (only the single separator space is removed),
  // otherwise typing a space inside the notes field gets stripped on every keystroke.

...
function RecurringPage() {
  ...
  const { data: list } = useQuery({
    ...
    enabled: !!me?.id,
    queryFn: async () => {
      const { data } = await supabase.from("business_recurring_orders").select("*").eq("business_id", me!.id).order("created_at", { ascending: false });
      return data ?? [];
    },
  });
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (editing) {
        const { error } = await supabase.from("business_recurring_orders").update(payload).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("business_recurring_orders").insert(payload);
        if (error) throw error;
      }
    },
  ...
  const toggle = useMutation({
    mutationFn: async ({ id, active }: { id: string; active: boolean }) => {
      const { error } = await supabase.from("business_recurring_orders").update({ active }).eq("id", id);
      if (error) throw error;
    },
  ...
  const del = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("business_recurring_orders").delete().eq("id", id);
      if (error) throw error;
    },

...
function WalletPage() {
  ...
  const recharge = useMutation({
    mutationFn: async () => {
      ...
      // TODO: replace with PayPal Orders API once credentials are added.
      const { error } = await supabase.from("wallet_transactions" as any).insert([
        { business_id: me.id, amount, kind: "recharge", description: `טעינה ₪${amount}` },
        ...(bonusVal > 0 ? [{ business_id: me.id, amount: bonusVal, kind: "bonus", description: `בונוס טעינה ${pct}% — ₪${bonusVal}` }] : []),
      ]);

...
function OrdersPage() {
  ...
  const reorder = useMutation({
    mutationFn: async (jobId: string) => {
      ...
      const { data: full } = await supabase.from("jobs").select("*").eq("id", jobId).eq("customer_id", me.id).maybeSingle();
      if (!full) throw new Error("not found");
      const { id: _i, created_at, updated_at, job_number, status, selected_courier_id, selected_quote_id, recipient_tracking_token, ...rest } = full as any;
      const { data, error } = await supabase.from("jobs").insert({ ...rest, customer_id: me.id, status: "נשלחה לשליחים" }).select("id").single();
  ...
  const { data: jobs } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("jobs")
        .select("id, job_number, job_type, pickup_area, dropoff_area, status, courier_step, payment, created_at, job_date, job_time, selected_courier_id, couriers:selected_courier_id(full_name)")
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`business-orders-live-${me.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "jobs", filter: `customer_id=eq.${me.id}` }, () => {
        qc.invalidateQueries({ queryKey: ["business-orders"] });
      })
      .subscribe();

...
async function fetchCouriers(): Promise<CourierRow[]> {
  const { data, error } = await supabase
    .from("couriers")
    .select("*, courier_tags(tag:tags(id,name,color))")
  ...
  const mut = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.from("couriers").insert({
        ...
        vehicle_type: form.vehicle_type as "קטנוע",
        invoice_status: form.invoice_status as "לא",
      ...
      if (error) throw error;
      if (data?.id) await reclassify({ data: { id: data.id } });
    },
    onSuccess: () => {
...
function NoteDialog({ courier, open, onOpenChange }: { courier: CourierRow | null; open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("couriers").update({ notes: merged }).eq("id", courier.id);
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["couriers"] });
...
function CouriersPage() {
  ...
  useEffect(() => {
    const ch = supabase
      .channel("admin-couriers-live")
      .on("postgres_changes", { event: "*", schema: "public", table: "couriers" }, () => {
        qc.invalidateQueries({ queryKey: ["couriers"] });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "courier_stats" }, () => {
        qc.invalidateQueries({ queryKey: ["couriers"] });
      })
      .subscribe();
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: CourierStatus }) => {
      ...
      const { error } = await supabase.from("couriers").update({ courier_status: status }).eq("id", id);
      if (error) throw error;

...
function BusinessProfile() {
  ...
  useEffect(() => {
    if (!id) return;
    const ch = supabase
      .channel(`admin-business-${id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "customers", filter: `id=eq.${id}` }, () => {
        qc.invalidateQueries({ queryKey: ["admin-business", id] });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "jobs", filter: `customer_id=eq.${id}` }, () => {
        qc.invalidateQueries({ queryKey: ["admin-business-jobs", id] });
      })
      .subscribe();
                ...
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    const { error } = await supabase
                      .from("customers")
                      .update({ notify_recipient_allowed: allowed, ...(allowed ? {} : { notify_recipient_enabled: false }) })
                      .eq("id", id);

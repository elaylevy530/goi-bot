...
function SendJobPage() {
  ...
  const { data: areas = [] } = useQuery({
    queryKey: ["areas"],
    queryFn: async () => {
      const { data } = await supabase.from("areas").select("name").eq("is_active", true).order("name");
      return data ?? [];
    },
  });
  ...
  useEffect(() => {
    const ch = supabase
      .channel("admin-send-job")
      .on("postgres_changes", { event: "*", schema: "public", table: "couriers" }, () => {
        qc.invalidateQueries({ queryKey: ["couriers-all"] });
      })
...
${description ? `הערות: ${description}\n` : ""}רוצה לקחת? השב 1.`;
  ...
  const createJob = useMutation({
    mutationFn: async () => {
      ...
      // Geocode synchronously so dispatch can match by GPS proximity immediately.
      try { await geocode({ data: { jobId: data.id } }); } catch (e) { console.error("geocode", e); }
      // Run the real dispatch engine: GPS+area matching, offer_events, in-app offers, Push, WhatsApp.
      try {
        const res = await dispatch({ data: { jobId: data.id } });
        if (res?.sent) toast.success(`שודר ל-${res.sent} שליחים`);
        else {
          await supabase.from("jobs").update({ status: "טיוטה" }).eq("id", data.id);
          toast.message("העבודה נוצרה — אין כרגע שליחים תואמים זמינים");
        }
      } catch (e) {
  ...
  const logSent = useMutation({
    mutationFn: async ({ courierId, phone }: { courierId: string; phone: string }) => {
      const { error } = await supabase.from("whatsapp_messages").insert({
        phone, courier_id: courierId, job_id: createdJobId,
        direction: "outbound", body: message, delivery_status: "sent",
      });
      if (error) throw error;
    },
    onSuccess: (_, vars) => {

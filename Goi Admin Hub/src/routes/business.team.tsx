...
function TeamPage() {
  ...
  const { data: members = [], isLoading } = useQuery({
    ...
    enabled: !!me?.id,
    queryFn: async () => {
      const { data, error } = await supabase.from("business_team_members" as never)
        .select("*").eq("business_id", me!.id)
  ...
  const invite = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("business_team_members" as never).insert({
        business_id: me.id,
        name: form.name.trim(),
        phone,
        role: form.role,
  ...
  const changeRole = useMutation({
    mutationFn: async ({ id, role }: { id: string; role: Role }) => {
      const { error } = await supabase.from("business_team_members" as never)
        .update({ role } as never).eq("id", id);
      if (error) throw error;
    },
  ...
  const remove = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("business_team_members" as never).delete().eq("id", id);
      if (error) throw error;
    },

...
function ProfilePage() {
  ...
  const { data: stats } = useQuery({
    ...
    enabled: !!me?.id,
    queryFn: async () => {
      const [{ data: statsRow }, { data: outcomes }] = await Promise.all([
        supabase.from("courier_stats").select("avg_rating").eq("courier_id", me!.id).maybeSingle(),
        supabase
          .from("job_outcomes")
          .select("customer_rating, delivered_at, was_cancelled")
          .eq("courier_id", me!.id)
          .not("customer_rating", "is", null),
      ]);
      const liveRatings = ((outcomes ?? []) as any[])
  ...
  const { data: avatarUrl } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.storage.from("courier-avatars").createSignedUrl(avatarPath, 60 * 60 * 24 * 7);
  ...
  const { data: balance = 0 } = useQuery({
    ...
    enabled: !!me?.id,
    queryFn: async () => {
      const [{ data: outs }, { data: wds }] = await Promise.all([
        supabase.from("job_outcomes").select("tip_amount, was_cancelled, delivered_at, jobs(payment)").eq("courier_id", me!.id),
        supabase.from("withdrawal_requests").select("amount,status").eq("courier_id", me!.id),
      ]);
      const earned = (outs ?? [])
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`profile-hub-${me.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "courier_stats", filter: `courier_id=eq.${me.id}` }, () => {
        qc.invalidateQueries({ queryKey: ["profile-hub-stats", me.id] });
      })
      .on("postgres_changes", { event: "*", schema: "public", table: "job_outcomes", filter: `courier_id=eq.${me.id}` }, () => {
        qc.invalidateQueries({ queryKey: ["profile-hub-stats", me.id] });
        qc.invalidateQueries({ queryKey: ["profile-hub-balance", me.id] });
      })

...
function BusinessQuotesPage() {
  ...
  const { data: jobs = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("jobs")
        .select("id, job_number, job_type, pickup_address, pickup_area, dropoff_address, dropoff_area, job_date, job_time, description, status, selected_quote_id, created_at, quote_deadline_at")
  ...
  const { data: quotes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("job_quotes")
        .select("*, couriers(full_name, whatsapp_phone, vehicle_type, base_city)")
        .in("job_id", jobIds)
  ...
  const select = useMutation({
    mutationFn: async (quoteId: string) => {
      const { data, error } = await supabase.rpc("select_job_quote", { _quote_id: quoteId });
  ...
  const cancelJob = useMutation({
    mutationFn: async (jobId: string) => {
      const { error } = await supabase.from("jobs").update({ status: "בוטלה" as never }).eq("id", jobId);
      if (error) throw new Error(error.message);
    },
    onSuccess: () => {
  ...
  // Realtime: new quotes incoming
  useEffect(() => {
    if (!me?.id) return;
    const ch = supabase.channel(`business-quotes-${me.id}`)

...
/** Throw an AppError when a Supabase result has error or is empty (optional). */
export function ensureSupabase<T>(
  result: { data: T | null; error: { message: string; code?: string } | null },
  scope: string,
): T {
  if (result.error) {
    throw new AppError("internal", {
      internalMessage: `[${scope}] supabase error: ${result.error.message}`,
      cause: result.error,
    });
  }
